...
import type { LayoutState, LayoutThemeType, LayoutType, LayoutOffcanvasStatesType, OffcanvasControlType } from '@/types/layout'
...
export const LayoutProvider = ({ children }: ChildrenType) => {
  const [settings, setSettings] = useLocalStorage<LayoutState>('__SMART_ADMIN_NEXT_CONFIG__', INIT_STATE)
  const [offcanvasStates, setOffcanvasStates] = useState<LayoutOffcanvasStatesType>({ showCustomizer: false })
  ...
  const changeThemeStyle = useCallback(
    (themeId: string) => {
      ...
      if (themeId === 'default') {
        ...
        setSettings((prev: any) => ({ ...prev, selectedTheme: themeId }))
      }
    },
    [setSettings],
  )
