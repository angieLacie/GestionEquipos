...
const AppMenu = ({ menuItems, openKeys }: { menuItems: MenuItemType[]; openKeys?: Set<string> }) => {
    ...
    <ul className="nav-menu d-flex">
      {menuItems.map((item) =>
        item.isTitle ? (
          ...
          <MenuItemWithChildren key={item.key} item={item} openMenuKey={openMenuKey} setOpenMenuKey={setOpenMenuKey} openKeys={openKeys} />
        ) : (
